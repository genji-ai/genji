...
export const ScrollHeadFront: ScrollComponent = ({ rect, zIndex }: ScrollProps) => {
  ...
  return (
    <DisplayContainer displayProps={{ rect, zIndex }}>
      ...
      >
        <path d={pathData} fill='url(#head-front-gradient)' />

        <defs>
          <linearGradient
            id='head-front-gradient'
            x1={`${rect.w / 2}`}
            y1='0'
            x2={`${rect.w / 2}`}
            y2='64'
          ...
          >
            <stop stopColor='#FAF3D4' />
            <stop offset='0.5' stopColor='#FCF7DF' />
            <stop offset='1' stopColor='#BAA48E' />
          </linearGradient>
        </defs>
      </svg>
    </DisplayContainer>
